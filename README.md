...
The tripwire for this specific failure: FD001 test RMSE under 11 means the split leaked; a real model on this dataset cannot do that well. It's a smell test only for the "too good" direction; landing above a published range isn't automatically suspicious.
...
That decision has a real, quantified cost: since the model never sees a training example labeled above 125, it can't confidently predict much higher than that either. For any test engine whose recorded history happens to stop while it's still well above the cap, the model's prediction is capped by what it learned, and the true answer isn't. That's an unavoidable blind spot for it, and one that most published C-MAPSS work also handles this exact way.

## Headline results

1. **FD001 test RMSE, final model (LSTM), mean across 5 seeds: 15.03 ± 0.86.** The LSTM was chosen as the final model because it beats XGBoost's RMSE on every one of the four datasets, with zero hyperparameter tuning. Reported as a mean and standard deviation across 5 seeds, not a single number, because TensorFlow CPU training isn't fully deterministic even with a fixed seed; see `docs/dataset-reference.md` for the individual per-seed values and the investigation.
2. **Reduction in late predictions from the RUL cap (XGBoost): 65% of engines predicted late down to 49%, a 16-point drop (about 25% relative).** See the ablation below for how it's computed.
...
| FD001 | LSTM | 15.03 ± 0.86¹ | 15.72 | 470.83 |
...
¹ Mean ± standard deviation across 5 seeds, per the headline above. Every other value in this table, including FD001 LSTM's own raw-label RMSE and NASA score, is a single run at seed 42; those two would drift by a similar margin on a rerun, since TensorFlow CPU training isn't fully deterministic (see `docs/dataset-reference.md`).

FD001/FD003 barely move between the two columns; FD002/FD004 jump by roughly 12 cycles. That isn't a harder-model problem: FD002/FD004 have roughly double the share of test engines truncated while still well above the 125-cycle cap (22%/27% vs. 11%/15% for FD001/FD003), exactly the scenario the cap can't handle, and a full diagnostic pass ruled out a pipeline bug (regime/scaler handoff, short-sequence handling, residual distribution; see `notebooks/eda_fd002_fd004_gap_investigation.ipynb`).

One more check worth calling out: restricting scoring to test engines whose true RUL already sits at or below 125, where capped and raw labels are identical, still puts FD002/FD004's LSTM around 13.6-14.0, modestly ahead of FD001's 14.4 on the same subset. That's a genuinely interesting result on its own (likely the per-condition regime normalization earning its keep, plus FD002 simply having 2.6x FD001's training data), not just an artifact of the cap creating "free" points.

### RUL cap ablation: how much does capping actually help?

This ablation uses XGBoost, not the LSTM: an uncapped-RUL baseline is trained with the *identical* XGBoost model and features; only the training target differs (raw RUL vs. capped RUL). That isolates the cap's effect from everything else. The main comparison is the plain share of test engines predicted late (capped model vs. uncapped baseline):
...
`requirements.txt` lists the direct dependencies (numpy, pandas, scikit-learn, scipy, xgboost, tensorflow, matplotlib) with the versions this project was built and run against. For an exact environment match, including transitive dependencies, use `requirements-freeze.txt` instead.
